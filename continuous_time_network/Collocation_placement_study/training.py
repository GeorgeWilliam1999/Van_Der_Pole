...
Axis 3 of the three-axis collocation follow-up (axis 1, density, lives in
../Collocation_study): at the fixed density of 20 points per unit time used
by every prior study, this study varies WHERE the points sit. Three
placements -- latin (the baseline Latin hypercube), uniform (deterministic
midpoints, no jitter) and anchored (the latin times plus 40 extra points
geometrically packed into (1e-3, 1]) -- crossed with horizons {14, 27},
seeds {0, 1, 2} and both arms. The anchored placement is the direct test of
the measured escape-gap loophole: the starved causal fits at T = 27 and 40
satisfy the anchor at t = 0 exactly, then jump to an impostor trajectory in
the unsampled gap before the first collocation point. If plugging that gap
matters, anchored runs will show it; if not, the loophole is not about the
first gap specifically. The latin baseline at T = 14 and 27 already exists
in ../Causal_weighting and ../Initial_pass and is imported at analysis time,
not rerun (the uniform placement is seed-independent, so seeds only vary its
network initialisation).

Everything except placement is copied unchanged from ../Collocation_study
(which copied it unchanged from the prior studies): architecture (4 hidden
layers x 50 tanh units), the unweighted protocol (full-batch L-BFGS, strong
Wolfe, up to 6 restarts x 200 iterations, early stop on two consecutive
sub-1% restarts) and the causal protocol (Adam front propagation with
weights recomputed from detached residuals every iteration, epsilon annealed
through {0.01, 0.1, 1, 10, 100} advancing once the minimum weight exceeds
0.99, 60,000-step budget, then the identical L-BFGS polish on the plain
unweighted loss).
...
(the completion marker), and is skipped on resubmission.
"""
...
def train_unweighted(horizon: float, placement: str, seed: int,
    ...
    t_c = torch.tensor(collocation_times(horizon, placement, seed)[:, None])
    y0 = torch.tensor([list(START)])
    history = _lbfgs_loop(model, t_c, y0, outer_steps, max_iter)
    return model, dict(history=history)


def train_causal(horizon: float, placement: str, seed: int,
    ...
    torch.manual_seed(seed)
    model = TrajectoryNetwork(horizon, WIDTH, DEPTH)
    t_np = collocation_times(horizon, placement, seed)
...
def run_one(arm: str, horizon: float, placement: str, seed: int,
           smoke: bool = False) -> dict:
    """Train and score one (arm, horizon, placement, seed) run.
    ...
    retrains, shrunk budgets) so a smoke run whose args coincide with a
    real job can never collide with or overwrite that job's real artifacts; smoke
    ...
    n_collocation = len(collocation_times(horizon, placement, seed))

    tic = time.perf_counter()
    if arm == "unweighted":
        model, info = train_unweighted(horizon, placement, seed,
                                       outer_steps=outer_steps, max_iter=max_iter)
    elif arm == "causal":
        model, info = train_causal(horizon, placement, seed, adam_steps=adam_steps,
                                   outer_steps=outer_steps, max_iter=max_iter)
    else:
    ...
    row = dict(tag=tag, arm=arm, horizon=horizon, placement=placement, seed=seed,
