...
Together these explain the pattern already measured in our earlier work: under the L-BFGS-only protocol at density 20 the unweighted objective fails past one period and the causally weighted objective past two, and no resource, neither network size nor collocation density nor placement nor a fourfold budget, moves the four-period wall. (Two qualifications from the convergence-assured runs of September 2026: with an Adam phase before the polish the unweighted objective succeeds in half the seeds at two periods, and in all of them at density 80; the four-period wall does not move.) Resources change the price of representing the truth; none of them changes the price of a splice, which is set by the structure of the objective.
...
1. The *local-stability factor* is the sum of the positive real parts of the eigenvalues of the Jacobian of $f$ at the network's output,
   ...
   It is positive only where the flow is locally unstable. For van der Pol the trace of $J$ is $1 - y_1^2$ and the determinant is $1 + 2 y_1 y_2$. Where the eigenvalues are a complex pair the factor equals $\max(1 - y_1^2, 0)$, positive only inside the band $|y_1| < 1$; where they are real, which happens where the determinant is negative ($y_1 y_2 < -\tfrac12$), one root is positive and the factor is that root, so the factor can also be positive outside the band. At the origin it equals $2 \times \tfrac12 = 1$. Wherever the network is moving the stillness factor below switches the product off, so on the loop the term is inert regardless.
